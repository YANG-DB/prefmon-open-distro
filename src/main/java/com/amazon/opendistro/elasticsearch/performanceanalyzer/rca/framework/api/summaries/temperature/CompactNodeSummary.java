...
public class CompactNodeSummary extends GenericSummary {
    ...
    public static CompactNodeSummary buildSummaryFromDatabase(Result<Record> records,
        ...
        final String nodeId =
            record.get(DSL.field(DSL.name(SQL_SCHEMA_CONSTANTS.NODE_ID_COL_NAME),
                String.class));
    ...
    public @Nullable
    NormalizedValue getTemperatureForDimension(TemperatureDimension dimension) {
    ...
    public static CompactNodeSummary buildNodeTemperatureProfileFromMessage(NodeTemperatureSummaryMessage message) {
        ...
        for (ResourceTemperatureMessage resourceMessage : message.getCpuTemperatureList()) {
            ...
            compactNodeTemperatureSummary.temperatureVector.updateTemperatureForDimension(dimension,
                    new NormalizedValue((short) resourceMessage.getMeanUsage()));
    ...
    public JsonElement toJson() {
        ...
        jsonObject.addProperty(SQL_SCHEMA_CONSTANTS.NODE_ID_COL_NAME, nodeId);
        jsonObject.addProperty(SQL_SCHEMA_CONSTANTS.HOST_IP_ADDRESS_COL_NAME, hostAddress);

        for (TemperatureDimension dimension : TemperatureDimension.values()) {
            NormalizedValue ret = temperatureVector.getTemperatureFor(dimension);
            if (ret == null) {
                ret = new NormalizedValue((short) 0);
            }
